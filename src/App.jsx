...
export const App = () => {
  ...
  useEffect(() => {
    Promise.all([api.getInfoUser(), api.getPosts()]).then(
      ([userData, postsData]) => {
        setUser(userData);
        setPosts(postsData);
      }
  ...
  function therePostLike({ _id, likes }) {
    ...
    api.likeStatusFun(_id, isLiked).then((newPost) => {
  ...
  return (
    <UserContext.Provider value={enterUser}>
      <Layout>
        ...
        <Content>
          ...
          <Routes>
            ...
            <Route
              path="/post/:postId"
              element={
                <PostPage enterUser={enterUser} therePostLike={therePostLike} />
              }
            />
            <Route
              path="/newpost"
              element={
                <NewPostPage
                  funEnterNewPost={enterNewPost}
                  formPostNewPost={api.postNewPost}
                />
              }
            />
        ...
        <Footer className="footer">Поделись своими знаниями</Footer>
      </Layout>
